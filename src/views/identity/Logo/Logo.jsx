...
const Logo = ({ data }) => {
  ...
  React.useEffect(() => {
    if (!ready && data) {
      const logoEle = data.elements.find((element) => element.type === 'logo-element');
      const doNotEle = data.elements.find((element) => element.type === 'do-not-element');
      setLogoArray(logoEle.content);
      setDoNotArr(doNotEle.content);
      setReady(true);
    }
  }, [ready, data]);
